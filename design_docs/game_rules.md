...
5. [抽取与品质系统](#抽取与品质系统)
6. [道具系统](#道具系统)
7. [人群系统（doom）](#人群系统doom)
8. [订单系统](#订单系统)
9. [菜篮与冰箱](#菜篮与冰箱)
10. [菜品与结算](#菜品与结算)
11. [配置面板（LIVE_CONFIG）](#配置面板live_config)
12. [附录：数值汇总表](#附录数值汇总表)
...
setup → 今日菜品揭晓（DishCard 浮层 + 菜品要求）
   ...
   ▼
每回合：
  wall_choice  3 店随机刷选 1 家进入（或点"回到餐厅"直接撤离）
  wall_reveal  揭晓墙面 modifier（当前无生效 modifier——纯市场）
  drawing      4×4 墙面抽取（见下）
  between_turns 抽奖结束；累积 doom；刷 1 个 2 选 1 订单事件；获得 1 个随机道具
   │
   ▼
...
候选卡下方有 **回到餐厅** 按钮（2026-04-22 从 between_turns 搬过来）——玩家可以选择不进任何市场、直接结束一天去做菜。
...
| `[0.15, 0.18)` | 抽数（gold） | **3%** | 🎫 |
| `[0.18, 0.23)` | 订单（order_cell） | **5%** | 📋 |
| `[0.23, 0.27)` | 炸弹（bomb） | **4%** | 💣 |
| `[0.27, 0.30)` | 道具（tool） | **3%** | 🧰（或具体道具图标） |
| `[0.30, 1.0)` | 留空 → 食材格（Phase 3） | **70%** |  |

道具格在生成时就已决定**具体哪个道具**（5 种等概率），墙上直接显示该道具图标。

### Phase 3：食材格

每个剩余空格独立放 1 个食材，彼此之间没有分组或形状关系。食材从**进入市场的大类下 12 个基础食材**（4 小类 × 3 个）均匀随机挑。食材格在墙上只显**品类名 + shortLabel**，**不显示品质**（品质在抽取时 roll；被透视后显示预 roll 出的品质色数字角标）。
...
默认抽取的品质概率 `LIVE_CONFIG.qualityWeights` = `{1:0.40, 2:0.30, 3:0.18, 4:0.09, 5:0.03}`（2026-04-22 紫 0.08→0.09、橙 0.04→0.03）。
...
| 炸弹 💣 | 摧毁周围 8 格 |
| 道具 🧰 | 获得该格预先指定的道具；放不下时弹替换/丢弃浮层（见道具系统）|
...
2026-04-22 加入。骨架是"**抽取前塑形**"——道具不改变抽取本身的规则，只在抽取前修改情境（信息 / 墙面 / 资源）。

### 道具栏

- 容量 3 格（`TOOL_CONFIG.capacity = 3`），独立于菜篮
- 允许重复持有（`allowDuplicates: true`）——同一 id 可占 2-3 格
- 显示在墙面下方的 Toolbar 组件
- 点击格子进入该道具的**激活模式**：ToolHintBar 顶栏提示「使用 X 中 — 点击 Y」，可取消

### 5 个道具（`TOOLS`）

| id | 名 | 图标 | 描述 | target |
|----|------|------|------|--------|
| `peek` | 透视 | 👁 | 揭示一行或一列所有食材的品质（预 roll 并锁定） | 一行/列 |
| `swap` | 换位 | 🔄 | 选择墙上任意 2 格交换位置 | 2 格 |
| `disperse` | 驱散 | 💨 | 丢弃墙上任意 1 格的内容 | 1 格 |
| `bomb_wall` | 炸墙 | 💥 | 炸掉一个 3×3 区域，重新填充 | 中心格 |
| `clear_inventory` | 清库换抽 | 🗑 | 丢弃 1 格菜篮食材，换 1 抽数 | 菜篮格 |

### 获得渠道（`TOOL_CONFIG`）

| 渠道 | 数量 | 时机 |
|------|------|------|
| 每天开局 | 3 个**互不相同**（`dayStartCount`） | startGame；批量一次 Popup 确认 |
| 每次离店（between_turns） | 1 个随机（`perWallExitCount`） | 进入 between_turns |
| 墙上道具格 🧰 | 1 个（具体道具在墙生成时确定） | 抽中道具格 |

### ToolGrantPopup（获得弹窗）

每次获得道具都弹出；开局 3 个批量一次；中途获得单个各弹一次。玩家点确认关闭，道具进 toolbar。

### ToolOverflowModal（道具栏满）

当 toolbar 3 格都占用且又要获得新道具：弹出浮层让玩家**替换**（选旧 3 格其中 1 个换掉）或**丢弃新的**（拒绝接收）。

### 使用过渡动画（300-600ms）

| 道具 | 动画 class | 视觉 |
|------|-----------|------|
| peek | `animate-tool-peek` | 蓝色扫光走过选中行/列 |
| swap | `animate-tool-swap` | 两格琥珀爆闪 |
| disperse | `animate-tool-disperse` | 目标格淡出 |
| bomb_wall | `animate-tool-bomb` | 3×3 橙红闪光 |
| clear_inventory | `animate-tool-clear-inv` | 菜篮格上浮淡出 |

### 透视特别说明

透视道具会把选中行/列所有 ingredient 格的 quality **预 roll 并锁定**——之后这些格被抽中时直接用这个值，不再重 roll。UI 上显示为对应品质色的数字角标（1-5）。
