...
### Phase 3：食材格 polyomino

`LIVE_CONFIG.shapeWeights` 决定形状大小权重（默认 `{1:60, 2:30, 3:10}`）——60% 放 1 格，30% 放 2 格（横/竖），10% 放 3 格（I3/L3）。同形状所有格**共享同一食材** + 同 `groupId`，视觉上是一块。放不下大形状会自动缩小到 1。

食材从**进入市场的大类下 16 个基础食材**均匀随机挑。食材格在墙上只显**品类名 + shortLabel**，**不显示品质**（品质在抽取时 roll）。
