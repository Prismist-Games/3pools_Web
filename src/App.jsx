                ...
                <div className="fixed inset-0 z-[150] flex items-center justify-center bg-black/70 backdrop-blur-sm animate-in fade-in">
                    <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl h-[85vh] flex flex-col overflow-hidden">
                        ...
                        <div className="flex-1 overflow-y-auto p-6 space-y-8">
                            ...
                            {/* Fate Dice Evacuation Difficulty Thresholds */}
                            <section className="bg-gradient-to-br from-indigo-50 to-violet-50 p-5 rounded-xl border-2 border-indigo-200">
                                <h4 className="text-lg font-bold mb-4 border-l-4 border-indigo-500 pl-3 flex items-center gap-2">
                                    🎲 撤离难度配置 — 每级所需骰子点数 (1-{config.emergency?.difficulty?.maxDifficulty || 10})
                                </h4>
                                <div className="text-xs text-slate-600 mb-4 bg-white/60 p-3 rounded-lg border border-indigo-100">
                                    每次撤离后难度提升，下一次撤离需要更多的命运骰子点数。
                                    <br />
                                    <span className="text-indigo-600 font-bold">💡 难度递增规则在上方"难度系统"中配置（初始难度、每次提升值、最大难度）</span>
                                </div>

                                <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
                                {Array.from({ length: config.emergency?.difficulty?.maxDifficulty || 10 }, (_, i) => i + 1).map(difficulty => {
                                    const threshold = config.emergency?.difficultyThresholds?.[difficulty] ?? '';
                                    return (
                                        <div key={difficulty} className="bg-white rounded-lg border border-indigo-200 shadow-sm p-3 flex flex-col items-center gap-2">
                                            ...
                                            <input
                                                ...
                                                min="1"
                                                max="36"
                                                className="w-full p-2 border border-indigo-200 rounded font-mono text-lg text-center font-bold"
                                                placeholder="🎲"
                                                value={threshold}
                                                onChange={(e) => setConfig({
                                                    ...config,
                                                    emergency: {
                                                        ...
                                                        difficultyThresholds: {
                                                            ...(config.emergency?.difficultyThresholds || {}),
