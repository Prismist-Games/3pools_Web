...
export const SkillSelectionModal = ({ candidates, onSelect, currentSkills, onReplace }) => {
    ...
    return (
        <div className={`fixed inset-0 z-[200] flex items-center justify-center transition-colors duration-200 ${isPeeking ? 'bg-transparent' : 'bg-black/80 backdrop-blur-md'}`}>
            {/* 偷看按钮 - 始终可见 */}
            <button
                className="absolute top-4 right-4 z-[210] bg-white text-stone-800 p-3 rounded-full shadow-lg font-bold flex items-center gap-2 hover:bg-stone-100 active:scale-95 transition-all cursor-pointer ring-2 ring-stone-200"
            ...
            <div className={`w-full max-w-5xl p-6 lg:p-8 flex flex-col items-center h-[90vh] overflow-y-auto transition-opacity duration-200 ${isPeeking ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}>
                ...
                <p className="text-stone-300 mb-8 font-bold text-center">
                ...
                <div className="flex flex-col gap-8 w-full">
                    ...
                    <div className="w-full">
                        <div className="text-sm font-bold text-stone-400 uppercase tracking-widest mb-4 flex items-center gap-2">
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <div className="text-center">
                                            <h3 className="text-lg font-bold text-stone-800">{t(skill.name)}</h3>
                                            <p className="text-xs text-stone-500 leading-relaxed mt-1">{t(skill.desc)}</p>
                                        </div>
                                        {!isReplacing && <span className="mt-2 text-xs font-bold text-sky-500 uppercase tracking-wider opacity-0 group-hover:opacity-100 transition-opacity">{t("点击获取")}</span>}
                                    </button>
                                )
                            })}
                        </div>
                    ...
                    <div className="w-full p-6 rounded-3xl bg-[#2D2A26]/80 border border-stone-700">
                        <div className="text-sm font-bold text-stone-400 uppercase tracking-widest mb-4 flex items-center gap-2">
                            {isReplacing ? <><Trash2 size={16} /> {t("选择要遗忘的旧技能")}</> : <><Info size={16} /> {t("当前已拥有技能")}</>}
                        </div>

                        {currentSkills.length === 0 ? (
                            <div className="text-stone-500 italic text-center py-4">{t("暂无技能")}</div>
                        ) : (
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                            ...
                                            className={`
                                                ...
                                                ${isReplacing
                                                    ? (targetOldSkillId === skill?.id
                                                        ...
                                                        : 'bg-stone-700 border-stone-600 text-stone-300 hover:bg-stone-600 cursor-pointer')
                                                    : 'bg-stone-700/50 border-stone-600 text-stone-400 cursor-default opacity-80' // 只读样式
                                                }
                                            `}
                                        >
                                            <div className={`w-12 h-12 rounded-full flex items-center justify-center ${skill?.color || 'bg-stone-500'} bg-opacity-90 shadow-sm`}>
                                                <SkillIcon size={24} />
                                            </div>
                                            <div className="text-center">
                ...
                <div className="mt-8 flex gap-4 w-full justify-center">
                    <button
                        ...
                        className="px-8 py-3 rounded-full border-2 border-stone-500 text-stone-300 hover:bg-stone-700 hover:text-white transition-all font-bold uppercase tracking-wider"
                    >
                        {t("放弃新技能")}
                    </button>
